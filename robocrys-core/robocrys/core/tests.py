...
from robocrys.core.util import load_condensed_structure_json
    ...
    for _fn in os.listdir(_structures_dir):
        if ".json.gz" in _fn:
            _test_structures[_fn.split(".")[0]] = loadfn(
                os.path.join(_structures_dir, _fn), cls=MontyDecoder
            )
    ...
    for _fn in os.listdir(_condensed_structures_dir):
        if ".json.gz" in _fn:
            _test_condensed_structures[
                ...
                os.path.join(_condensed_structures_dir, _fn)
            )
